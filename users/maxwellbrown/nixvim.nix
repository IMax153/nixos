...
let
  ...
  aider = inputs.aider.packages.${pkgs.system}.default;
  aiderConfigFile = config.sops.templates.".aider.conf.yml".path;

  nixvim = inputs.nixvim.packages.${pkgs.system}.default.extend {
    extraConfigLua =
      # lua
      ''
        require("nvim_aider").setup({
          args = {
            "--config ${aiderConfigFile}"
          }
        })
      '';
  };
in
{
  ...
  sops = {
    ...
    templates.".aider.conf.yml".content = ''
      ##########################################################
      ...
      openai-api-key: ${config.sops.placeholder.openai-api-key}
